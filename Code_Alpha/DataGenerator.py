...
class DataGeneration():
	def __init__(self,
		...
		self.partitionData_train_validation();
	...
	def partitionData_train_validation(self):
		...
		for cameraModel in self.dataDict_cameras_imgPaths.keys():
			self.trainDataDict[cameraModel] = self.dataDict_cameras_imgPaths[cameraModel][0:250]
			self.validationDataDict[cameraModel] = self.dataDict_cameras_imgPaths[cameraModel][250:]
	...
	def readData2populateDict(self):
		...
		for cameraModel in self.dataDict_cameras_imgPaths.keys():
			self.dataDict_cameras_imgPaths[cameraModel] = np.sort(self.dataDict_cameras_imgPaths[cameraModel])
		
	def dataGeneratefromDirectory(self,
		data_dict_camera_image=None,
		patch_size=(64,64),
		num_patch= 32,
		num_class= 10,
		color_mode='rgb',
		...
		while True:
			batch_labels = [];
			if ( save_prefix=='Train_Aug'):
				pdb.set_trace()
			data_label =self.patch_generation(patch_size,num_patch,num_class,data_dict_camera_image,save_data,num_patch_to_save,
	...
	def patch_generation(self,patch_size,num_patch,num_class,image_dict,
		...
		if num_patch_to_save is not 'SAVE_ALL':
			if ( self.num_batch_done > num_patch_to_save):
				save_data = False;
		#pdb.set_trace()
		patch_needto_compensated =0;
		patch_image_label=[];image_name=[];save_image_count=0;
		for camera_model in image_dict.keys():
			...
			if( patch_from_each_class < num_images):
				...
				for index in randomIndexList:
					...
					H,W,C = img.shape;
					patch_count=0;	fail_count =0;
					#pdb.set_trace()
					while(patch_count < num_patch_per_class_image):
						#print('computing : {}'.format(patch_count));
						if( fail_count > 2*num_patch_per_class_image):
							patch_needto_compensated = patch_needto_compensated +( num_patch_per_class_image - patch_count)
							break;
						...
						if score >= threshold_to_decide_patch:
							...
							if save_data==True:
								...
								file_name = save_to_dir+'\\'+save_prefix+'_'+camera_model+'_'+imageName+'_Loc_X_'+str(X)+'_Y_'+str(Y)+'_{0:2f}'.format(score)+'.'+save_format;
				...
				while(patch_count_valid < patch_from_each_class):
					...
					if( fail_count > 2*patch_from_each_class):
						patch_needto_compensated = patch_needto_compensated +( patch_from_each_class - patch_count_valid)
						break;
		...
		patch_count_left =0;
		random_patch_left = random_patch_left + patch_needto_compensated
	...
	def random_transform(self, x):
		...
		if self.data_format == 'channels_last': 
			...
			image_col_w_axis =2;
			image_chn_axis =0;
		cv2.imwrite('./input.jpg',x); print('...........start....');
		jpeg_comp_flag = False;
		if self.flipH:
			if np.random.random() < 0.5:
				x = flip_axis(x, image_col_w_axis); print('flip_h');
		cv2.imwrite('./input_post_fliph.jpg',x);		
		if self.flipV:
			if np.random.random() < 0.5:
				x = flip_axis(x, image_row_h_axis);print('flip_v');
		cv2.imwrite('./input_post_flipv.jpg',x);
		if self.Rotation:
			if np.random.random() < 0.5:
				if np.random.random() < 0.5:
					x= Rotate(x,'R90');print('rot90');
				else:
					x= Rotate(x,'R270');print('rot270');
		cv2.imwrite('./input_post_rotation.jpg',x);
		if self.JPEG_QF_Compress:
			if np.random.random() < 0.5:
				jpeg_comp_flag =True;
				if np.random.random() < 0.5:
					x = JpegCompression_withImage_valid(x,70);print('jpeg_70');
				else:
					x = JpegCompression_withImage_valid(x,90);print('jpeg_90');
		cv2.imwrite('./input_post_jpeg.jpg',x);					
		if (self.RESIZE_BC) and (not jpeg_comp_flag):
			if np.random.random() < 0.5:
				factor = np.random.choice([0.5, 0.8, 1.5, 2]);
					...
					#pdb.set_trace()
				x = np.array(x);
				x= Resize_Bicubic(x,factor); print('bicubit.factor ={}'.format(factor));
		cv2.imwrite('./input_post_resisebc.jpg',x);		
		if self.GammaCorr:
			if np.random.random() < 0.5:
				if np.any(x < 0):
					pdb.set_trace()
				if np.random.random() < 0.5:
					x = GammaCorr(x,0.8);print('gama_0.5');
				else:
					x = GammaCorr(x,1.2);print('gamma_1p2');
		cv2.imwrite('./input_post_gammaCor.jpg',x);
		if self.Rescale_Intensity:
			if np.random.random() <0.5:
				x = rescale_intesity(x);print('inte_scale');
		cv2.imwrite('./input_post_recleIntesnsity.jpg',x);
		if self.shear_range:
			shear = np.random.uniform(-self.shear_range, self.shear_range);print('shear');
		else:
			shear = 0
		...
		cv2.imwrite('./input_post_shear.jpg',x); print('....end..');
		return x;
